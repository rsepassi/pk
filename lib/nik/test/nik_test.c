...
void test_nik(void) {
  ...
  CHECK0(crypto_box_keypair((u8*)&alice.pk, (u8*)&alice.sk));
  CryptoKxKeypair bob;
  CHECK0(crypto_box_keypair((u8*)&bob.pk, (u8*)&bob.sk));
  ...
  NIK_Handshake1 hs1;
  {
    ...
    CHECK0(nikhs_handshake_start(&alice_state, alice_keys, &hs1));
  }
  ...
  NIK_Handshake2 hs2;
  {
    ...
    CHECK0(nikhs_handshake_responder_finish(&bob_state, bob_keys, &hs1, &hs2,
                                            &bob_secret));
  }
  ...
  // Alice's state
  {
    ...
    NIK_SharedSecret s;
    CHECK(nikhs_handshake_finish(&alice_state, &bogus, &s) ==
          NIK_ErrFailedVerify);
  }

  // Alice completes the handshake
  NIK_SharedSecret alice_secret;
  CHECK0(nikhs_handshake_finish(&alice_state, &hs2, &alice_secret));
