...
u8 crypto_seed_new_user(const CryptoSeed *seed, CryptoUserState *user) {
  // Identity key
  if (crypto_sign_seed_keypair((u8 *)&user->pub.sign, (u8 *)&user->sec.sign,
                               (u8 *)seed))
    return 1;

  // sk = seed + pk
  DCHECK(sodium_memcmp((u8 *)&user->sec.sign.seed, (u8 *)seed,
                       sizeof(CryptoSeed)) == 0);
  DCHECK(sodium_memcmp((u8 *)&user->sec.sign.pk, (u8 *)&user->pub.sign,
                       sizeof(CryptoSignPK)) == 0);

  // Identity kx key
  if (crypto_kx_seed_keypair((u8 *)&user->pub.kx, (u8 *)&user->sec.kx,
                             (u8 *)seed))
    return 1;

  // Pre-shared keypair
  if (crypto_kx_keypair((u8 *)&user->pub.kx_preshare,
                        (u8 *)&user->sec.kx_preshare))
    return 1;

  // Sign kx key
  if (crypto_sign_detached((u8 *)&user->pub.kx_sig, 0, // signature
                           (u8 *)&user->pub.kx,
  ...
  // Sign pre-shared key
  if (crypto_sign_detached((u8 *)&user->pub.kx_preshare_sig, 0, // signature
                           (u8 *)&user->pub.kx_preshare,
                           sizeof(CryptoKxPK),   // input message
                           (u8 *)&user->sec.sign // signing key
                           ))
    return 1;
...
// Alice -> Bob
static u8 crypto_x3dh_initiate(const CryptoUserState *A,
                               const CryptoUserPState *B, CryptoX3DHInit *out) {
  // Alice verifies Bob's kx key
  if (crypto_sign_verify_detached((u8 *)&B->kx_sig, (u8 *)&B->kx,
                                  sizeof(CryptoKxPK),
...
// Bob <- Alice
static u8 crypto_x3dh_reply(const CryptoUserState *B,
                            const CryptoX3DHFirstMessageHeaderAuth *A,
                            CryptoKxTx *out) {
  // Bob checks that the message is intended for him
  if (sodium_memcmp((u8 *)&B->pub.kx, (u8 *)&A->kx_B, sizeof(CryptoKxTx)))
    return 1;
  ...
  if (crypto_sign_verify_detached((u8 *)&A->kx_sig, (u8 *)&A->kx,
                                  sizeof(CryptoKxPK),
...
u64 crypto_x3dh_first_msg_len(u64 plaintxt_len) {
  u64 ciphertxt_len =
      plaintxt_len + crypto_secretstream_xchacha20poly1305_ABYTES;
...
u8 crypto_x3dh_first_msg(const CryptoUserState *A, const CryptoUserPState *B,
  ...
  if (out->len != crypto_x3dh_first_msg_len(plaintxt.len))
    return 1;
  ...
  CryptoX3DHFirstMessageHeader *header =
      (CryptoX3DHFirstMessageHeader *)out->buf;
  ...
  if (crypto_secretstream_xchacha20poly1305_init_push(
          &state, header->header, (u8 *)&A_x3dh_init.session_key))
    return 1;
  if (crypto_secretstream_xchacha20poly1305_push(
          &state, ciphertxt, NULL, plaintxt.buf, plaintxt.len,
...
u8 crypto_x3dh_first_msg_parse(const Str msg,
                               CryptoX3DHFirstMessageHeader **header,
                               Str *ciphertxt) {
  if (msg.len < sizeof(CryptoX3DHFirstMessageHeader))
    return 1;
  *header = (CryptoX3DHFirstMessageHeader *)(msg.buf);
  ...
  if (msg.len != sizeof(CryptoX3DHFirstMessageHeader) + ciphertxt->len)
    return 1;
  if (ciphertxt->len < crypto_secretstream_xchacha20poly1305_ABYTES)
    return 1;
...
u8 crypto_x3dh_first_msg_recv(const CryptoUserState *B,
  ...
  if (plaintxt->len != crypto_plaintxt_len(ciphertxt.len))
    return 1;
  CryptoKxTx B_x3dh_session_key;
  if (crypto_x3dh_reply(B, &header->auth, &B_x3dh_session_key))
    return 1;
  crypto_secretstream_xchacha20poly1305_state state;
  if (crypto_secretstream_xchacha20poly1305_init_pull(
          &state, header->header, (u8 *)&B_x3dh_session_key))
    return 1;

  u8 tag;
  if (crypto_secretstream_xchacha20poly1305_pull(
          &state, plaintxt->buf, 0, &tag, ciphertxt.buf, ciphertxt.len,
          (u8 *)&header->auth, sizeof(CryptoX3DHFirstMessageHeaderAuth)))
    return 1;
