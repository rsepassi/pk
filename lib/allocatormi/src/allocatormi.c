...
Allocator allocatormi_allocator(void) {
  return (Allocator){0, alloc, 0};
}

Allocator allocatormi_heap(void) {
  return allocatormi_heap_ex(mi_heap_new());
}
...
Allocator allocatormi_arena(Bytes arena, bool iszero) {
  mi_arena_id_t id;
  CHECK(mi_manage_os_memory_ex(arena.buf, arena.len, false, false, iszero, -1, true, &id));
...
Bytes allocatormi_block_alloc(usize n) {
  u64 sz = 1 << 26;  // 64MiB
  Bytes x = {sz * n, mmap(NULL, sz * n, PROT_READ | PROT_WRITE, MAP_ANONYMOUS, -1, 0)};
  return x;
}

void allocatormi_block_free(Bytes blk) {
  munmap(blk.buf, blk.len);
}
